...
var BootstrapVueNextResolver = Object.assign(({ aliases = {}, directives = true, components = true } = {}) => {
	const compImports = parseActiveImports(typeof components === "boolean" ? { all: components } : components, require_BootstrapVueOptions.componentNames).reduce((map, name) => {
		map.set(name, `${bvKey}${require_BootstrapVueOptions.componentsWithExternalPath[name]}`);
		return map;
	}, /* @__PURE__ */ new Map());
	const dirImports = parseActiveImports(typeof directives === "boolean" ? { all: directives } : directives, require_BootstrapVueOptions.directiveNames).reduce((map, directive) => {
		const key = directive.toLowerCase().startsWith("v") ? directive : `v${directive}`;
		map.set(key, `${bvKey}${require_BootstrapVueOptions.directivesWithExternalPath[key]}`);
		return map;
	}, /* @__PURE__ */ new Map());
	return [{
		type: "component",
