...
//#endregion
//#region ../../node_modules/.pnpm/@vueuse+integrations@14.2.1_change-case@5.4.4_focus-trap@8.0.1_fuse.js@7.1.0_vue@3.5.31_typescript@5.9.3_/node_modules/@vueuse/integrations/dist/useFocusTrap-lXZ_YG-8.js
/**
* Reactive focus-trap
*
...
function useFocusTrap(target, options = {}) {
	...
	(0, vue.watch)((0, vue.computed)(() => {
		return require_dist.toArray((0, vue.toValue)(target)).map((el) => {
			const _el = (0, vue.toValue)(el);
			return typeof _el === "string" ? _el : require_dist.unrefElement(_el);
		}).filter(require_dist.notNullish);
	}), (els) => {
	...
	require_dist.tryOnScopeDispose(() => deactivate());
	return {
		hasFocus,
		isPaused,
		activate,
...
//#endregion
//#region src/composables/useActivatedFocusTrap.ts
var useActivatedFocusTrap = ({ element, isActive, noTrap, fallbackFocus, focus }, focusTrapOpts = {
	allowOutsideClick: true,
	fallbackFocus: () => fallbackFocus.ref.value || require_dom.getSafeDocument()?.body || "body",
	escapeDeactivates: false,
	clickOutsideDeactivates: false,
	initialFocus: focus,
	delayInitialFocus: false
}) => {
	const resolvedIsActive = (0, vue.readonly)((0, vue.toRef)(isActive));
	const resolvedNoTrap = (0, vue.readonly)((0, vue.toRef)(noTrap));
	const checkNeedsFallback = () => {
		return !(element.value?.querySelectorAll(`a, button, input, select, textarea, [tabindex]:not([tabindex="-1"]):not(.${fallbackFocus.classSelector})`))?.length;
	};
	const needsFallback = (0, vue.ref)(false);
	(0, vue.onMounted)(() => {
		needsFallback.value = checkNeedsFallback();
		require_dist.useMutationObserver(element, () => {
			needsFallback.value = checkNeedsFallback();
		}, {
			childList: true,
			subtree: true
	...
	(0, vue.watch)(resolvedIsActive, async (newValue) => {
		if (newValue && resolvedNoTrap.value === false) trap.activate();
		else trap.deactivate();
	});
	(0, vue.watch)(resolvedNoTrap, (newValue) => {
		if (newValue === true) trap.deactivate();
	});
	return { needsFallback: (0, vue.readonly)(needsFallback) };
};
//#endregion
//#region src/composables/useSafeScrollLock.ts
var prevousRightPadding = "";
var lockRegistry = /* @__PURE__ */ new Map();
var useSafeScrollLock = (isOpen, bodyScroll) => {
	const resolvedIsOpen = (0, vue.readonly)((0, vue.toRef)(isOpen));
	...
	*/
	const inverseBodyScrollingValue = (0, vue.computed)(() => !(0, vue.toValue)(bodyScroll));
	const isLocked = require_src_composables_useScrollLock_index.useScrollLock(require_dom.getSafeDocument()?.body ?? null, resolvedIsOpen.value && inverseBodyScrollingValue.value);
	(0, vue.onMounted)(() => {
		if (require_dom.getSafeDocument() === null) return;
		lockRegistry.set(id, false);
		(0, vue.watch)([resolvedIsOpen, inverseBodyScrollingValue], ([modelVal, bodyVal]) => {
			const doc = require_dom.getSafeDocument();
			const scrollBarGap = (require_dom.getSafeWindow()?.innerWidth ?? 0) - (doc?.documentElement.clientWidth ?? 0);
...
//#endregion
Object.defineProperty(exports, "useActivatedFocusTrap", {
	enumerable: true,
	get: function() {
		return useActivatedFocusTrap;
	}
});
Object.defineProperty(exports, "useSafeScrollLock", {
	enumerable: true,
	get: function() {
		return useSafeScrollLock;
	}
});

//# sourceMappingURL=useSafeScrollLock-7wmPH_Hy.js.map